...
class VrpFlowSolverService(
    ...
    fun solve(problemId: Long, uuid: UUID, solverName: String) {
        ...
        solverKeys[solverKey] = solverScope.launch {
            SolverFactory.createSolver(solverName, solverKey, SolverConfig(timeLimit)).also { solver ->
                ...
                solver.solutionFlow(currentSolution, currentMatrix)
                    ...
                    .onCompletion { ex ->
                        if (ex is CancellationException) {
                            broadcastSolution(VrpSolutionRequest(bestSolution, SolverStatus.TERMINATED, uuid))
                        }
                    }
    ...
    private fun broadcastSolution(solutionRequest: VrpSolutionRequest, clear: Boolean = false) {
        logger.info { "${solutionRequest.solverKey} - ${solutionRequest.status} - ${solutionRequest.solution.getTotalDistance()}" }
