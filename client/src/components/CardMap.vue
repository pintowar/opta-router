...
watchEffect(() => {
  const bounds: L.LatLngBounds = L
      .featureGroup((instance?.value?.stops || [])
      .map(e => new L.Marker([e.lat, e.lng])))
      .getBounds();    

  if (bounds.isValid()) {
    const tmp = [[bounds.getSouthWest().lat, bounds.getSouthWest().lng], [bounds.getNorthEast().lat, bounds.getNorthEast().lng]]
