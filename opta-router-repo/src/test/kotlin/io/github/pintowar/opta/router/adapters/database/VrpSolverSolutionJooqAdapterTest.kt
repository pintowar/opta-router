...
class VrpSolverSolutionJooqAdapterTest : FunSpec({
    ...
    suspend fun createProblemAndRequest(problemId: Long, requestKey: UUID, solverStatus: SolverStatus) {
        val now = Instant.now()
    ...
    test("upsertSolution should insert a new solution if none exists") {
        ...
        val solutionRequest = adapter.upsertSolution(problemId, SolverStatus.TERMINATED, routes, objective, false, requestKey)
    ...
    test("upsertSolution should update an existing solution") {
        ...
        val solutionRequest = adapter.upsertSolution(problemId, SolverStatus.TERMINATED, updatedRoutes, updatedObjective, false, requestKey)
    ...
    test("upsertSolution should clear solution and set status to NOT_SOLVED if clear is true") {
        ...
        val updatedRoutes = lastVrpSolution.routes
        val updatedObjective = lastVrpSolution.getTotalDistance().toDouble()
        val solutionRequest = adapter.upsertSolution(problemId, SolverStatus.NOT_SOLVED, updatedRoutes, updatedObjective, true, requestKey)
